import { Avatar, Card, CardActions, CardContent, CardHeader, CardMedia, Collapse, Grid, IconButton, ListItemText, Menu, MenuItem, Paper, Typography, withStyles } from "@material-ui/core"
...
class postCard extends Component {
  ...
  componentDidMount = () => {
    this.setState({ likeCount: this.props.likeCount, liked: this.props.is_liked })
  }
  getComment = () => {
  ...
  like = () => {
    this.setState({ likeCount: this.state.liked ? this.state.likeCount - 1 : this.state.likeCount + 1, liked: !this.state.liked })

    const postID = this.props.postID

    fetch(`${url}/post/${postID}/like`, { method: "POST", headers: { AUTHORIZATION: token } }).then(res => {
      res.json().then(d => {
        console.log(d)
  ...
  render() {
    const { classes, userName, postDate, postImgSrc, postText, avatar, isImg, post_uid } = this.props
    const { expanded, anchorEl, deletepost, cmnt, liked, likeCount, allCmnt } = this.state
        ...
        return (
          <Grid key={p.comment_id} className={classes.commentG} container justify="flex-start">
            ...
            <Paper className={classes.commentP}>
              <ListItemText primary={p.comment_by_userName} secondary={p.comment} />
            </Paper>
          </Grid>
        )
      })
        ...
        {!deletepost && (
          <Card className={classes.card}>
            <CardHeader
              ...
              subheader={postDate}
            />
            {isImg && <CardMedia className={classes.media} image={postImgSrc} title="#" />}
            <CardContent>
